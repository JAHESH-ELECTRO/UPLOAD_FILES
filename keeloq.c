...
void keeloq_encrypt(uint8_t *key, uint32_t *data, const unsigned short int nrounds) {
  uint32_t x;
...
void keeloq_decrypt(uint8_t *key, uint32_t *data, const unsigned short int nrounds) {
  uint32_t x;
  unsigned short int loop;
  uint8_t o, nlf_input, k, ki;
  ...
  for (loop = 0; loop < nrounds; loop++) {
    ...
    o = nlf(nlf_input);
    ki = (unsigned short int)(15 - loop) % 64;
    k = key[ki / 8] >> (ki % 8);
